...
/* classyc's stripped-down <limits.h> doesn't define this POSIX extension
 * (real glibc's does, via sys/types.h/bits/typesizes.h) -- vendored-copy
 * portability shim, not an upstream cejson/stringbuf change. */
#ifndef SSIZE_MAX
...
/* static inline, defined here (not gated behind STRINGBUF_IMPLEMENTATION
 * like the rest of this file's functions) for two reasons:
 *   1. static inline functions need their body visible in every TU that
 *      calls them -- gating the definition behind an opt-in macro would
 *      mean any includer that forgets STRINGBUF_IMPLEMENTATION gets a
 *      declared-but-never-defined function, an error the moment it's
 *      actually called (unlike the plain extern-linkage functions below,
 *      which only need ONE TU to define STRINGBUF_IMPLEMENTATION for the
 *      whole program to link against that one definition).
 *   2. These four are the ones actually called in a JSON-dump-style hot
 *      loop, often once per punctuation byte. A real (non-inlined)
 *      function call here is a measured, significant cost: ~68-88x
 *      slower dump time than a comparably-shaped ahead-of-time-compiled
 *      dump, before this change -- `static inline` lets an optimizing
 *      compiler fold the reserve-check-then-write sequence directly into
 *      the caller, the same way it already could for stringbuf_data/
 *      stringbuf_cstr/etc. just above. */
